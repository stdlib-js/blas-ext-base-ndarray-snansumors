...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof snansumors, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( snansumors.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of a one-dimensional ndarray, ignoring `NaN` values and using ordinary recursive summation', function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, NaN, 3.0, 0.0, -3.0, 3.0, NaN ] ); // eslint-disable-line max-len
	v = snansumors( [ vector( x, 11, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float32Array( [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0, NaN ] );
	v = snansumors( [ vector( x, 8, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float32Array( [ -4.0, NaN, -4.0 ] );
	v = snansumors( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( v, -8.0, 'returns expected value' );

	x = new Float32Array( [ NaN, 4.0 ] );
	v = snansumors( [ vector( x, 2, 1, 0 ) ] );
...
tape( 'if provided an empty ndarray, the function returns `0.0`', function test( t ) {
	...
	v = snansumors( [ vector( x, 0, 1, 0 ) ] );
	t.strictEqual( isPositiveZerof( v ), true, 'returns expected value' );

	t.end();
});

tape( 'if provided a ndarray containing a single element, the function returns that element', function test( t ) {
	var x;
	var v;
